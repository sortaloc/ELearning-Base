export namespace RegisterController {    
    ...
    class RegisterController {
        ...
        public registerUser = async (fields: Array<string>, body: RegisterData) => {
        ...
        public validasi = (tipe: Array<string>, data: validasi) => {
            return new Promise<Object>(async (resolve) => {
                let response = this.structure;
                try{
                    if(data.tipe === 'nik'){
                        let nik: any = NIK(data.value);
                    ...
                    let status: boolean = tipe.includes(data.tipe)
                    if(status){
                        let fields = `prl_${data.tipe}`;
                        const where: Object = { [fields] : data.value };
                        let res: Array<any> = await database.profile.allSelect(where);
                        if(res.length === 0){
                            response.state = true;
        ...
        public registerWhatsapp = async (body: any) => {
            return new Promise<Object>(async (resolve) => {
                let getText = body.Body;
                let response = this.structure;
                try{
                    getText = getText.trim().toLowerCase();
                    if(getText.indexOf('reg') > -1 || getText.indexOf('nexus') > -1){ //Jika ada
